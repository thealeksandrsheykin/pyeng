...
def select_data_from_db(connection,args):
      ...
      if check_args(connection,args):
            ...
            if not args:
                  ...
                  query = 'SELECT * from dhcp WHERE {} = \'{}\' and active = ?'.format(*args)
            for i in range(1,-1,-1):
                  cursor.execute(query,str(i))
                  data = cursor.fetchall()
                  if data:
                        print('\n{}:\n\n{}'.format(status[i],tabulate(data)))
                  else: pass
      else:
            return None

def output_data_to_screen(connection,act,inact):
      '''
      Функция выводи данные запроса к БД на экран. Функция ничего не возвращает.
      connection - соединение с БД.
      act - активные записи
      inact - неактивные записи
      '''
      if act:
            print('\nАктивные записи:\n{}'.format(tabulate(act)))
