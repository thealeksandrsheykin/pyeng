...
def remove_from_table_dhcp(connect):
      '''
      Функция удаляет данные из БД, которые были активны более 7 дней назад.
      Функция ничего не возвращает.
      connect - соединение с БД.
      '''
      now = datetime.today().replace(microsecond=0)
      week_ago = now - timedelta(days=7)
      for i in connect.execute('SELECT * from dhcp'):
            if str(week_ago) > i[6]:
                  print('Удаляю запись из таблицы dhcp: {}'.format(i))
                  connect.execute('DELETE from dhcp WHERE mac = \'{}\''.format(i[0]))
                  connect.commit()
            else: continue
      return None
...
if __name__ == '__main__':
      ...
      if connect:
            ...
            remove_from_table_dhcp(connect)
            close_db(connect)
      else: pass
