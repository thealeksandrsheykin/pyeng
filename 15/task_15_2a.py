...
def convert_to_dict(headers,list_of_tuple):
      '''
      Функция возвращает результат в виде списка словарей, где ключи - взяты из первого списка,
      а значения подставлены из второго.
      headers  - список названия полей
      list_of_tuple - список кортежей со значениями
      '''
      result = list()
      for i in list_of_tuple:
            tmp_list = list(zip(headers,i))
            result.append({key:value for key,value in tmp_list})
      return result


if __name__ == '__main__':
      list_of_tuple = parse_sh_ip_int_br('sh_ip_int_br.txt')
      print(convert_to_dict(headers,list_of_tuple))
